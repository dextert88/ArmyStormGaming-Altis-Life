...
switch (_code) do
{
	...
	case _mapKey:
	{
		switch (playerSide) do 
		{
			...
			case independent: {if(!visibleMap) then {[] spawn life_fnc_medicMarkers;}};			case civilian: {if(!visibleMap) then {[] spawn life_fnc_gangMarkers;}};	
		};
	};
	...
	{	
		...
		if(_shift && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3.5 && isPlayer cursorTarget && !(cursorTarget getVariable "Escorting") && (cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
		...
		if(_shift && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3.5 && isPlayer cursorTarget && !(cursorTarget getVariable "Escorting") && (cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			[] call life_fnc_unrestrainAction;
		}
	...
	case 38: 
	{
		...
		if(_shift && playerSide in [west,independent]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Hatchback_01_F","C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_01_F","I_Heli_light_03_unarmed_F","I_MRAP_03_hmg_F","I_MRAP_03_F","B_APC_Wheeled_01_cannon_F","B_MRAP_01_hmg_F"]) then {
